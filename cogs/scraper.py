...
class Scraper(commands.Cog, name="Archiving:"):
    ...
    async def scrape(self, path, channel, limit):
        '''
        Scrapes [limit] messages from a channel.
        Returns the message history scraped.
        '''
    ...
    async def store(self, ctx, channel, limit):
        '''
        Scrapes and stores channel messages and channel metadata, compressing the resulting archive.
        Returns the ID of the stored archive.
        '''
        # Limit of 0 -> no limit
        ...
        filename = hash(timestamp + str(channel.id)) % 1000000
        ...
        history = await self.scrape(path, channel, limit)
        ...
        # Return the ID of the saved archive
        return filename
